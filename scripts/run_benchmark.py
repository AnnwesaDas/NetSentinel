...
def run_once(binary, pcap, batch, flags):
    cmd = [binary, "-r", pcap, "-s", "-b", str(batch)] + flags
...
def main():
    ...
    runs = []
    print(f"== end to end: {len(modes)} modes x {len(batches)} batch sizes x {args.reps} reps ==")
    for batch in batches:
        for rep in range(args.reps):
            for mode, flags in modes:
                result = run_once(netsentinel, args.pcap, batch, flags)
                result.update(mode=mode, batch=batch, rep=rep)
                runs.append(result)
                print(f"  batch {batch:>5}  rep {rep + 1}/{args.reps}  {mode:<8} "
                      f"{result['pps']:>9} pkt/s  avg batch {result['avg_batch']:.1f}", flush=True)

    raw_path = os.path.join(out, "end_to_end_runs.csv")
    with open(raw_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["mode", "batch", "rep", "pps", "seconds",
    ...
    print(f"\n### End-to-end results\n\nMachine: {machine}. Capture: `{args.pcap}`, "
          f"median of {args.reps} runs.\n")
    print("| batch | mode | packets/sec (median) | min–max | avg batch | vs cpu |")
    print("|---:|---|---:|---:|---:|---:|")
    problems = []
    for batch in batches:
        cpu_median = statistics.median(r["pps"] for r in runs
                                       if r["mode"] == "cpu" and r["batch"] == batch)
        for mode, _ in modes:
            mine = [r for r in runs if r["mode"] == mode and r["batch"] == batch]
            pps = [r["pps"] for r in mine]
            median = statistics.median(pps)
            print(f"| {batch} | {mode} | {median:,.0f} | {min(pps):,}–{max(pps):,} | "
                  f"{statistics.median(r['avg_batch'] for r in mine):.1f} | "
                  f"{median / cpu_median:.2f}x |")
            if any(r["fallbacks"] for r in mine):
                problems.append(f"{mode} at batch {batch}: GPU batches fell back to the CPU")
    ...
    for kind in ("high_entropy", "signature"):
        ...
        if len(counts) == 1:
            ...
            print(f"\n{kind} alerts: {counts[0]:,} in every run ({ran}, all batch sizes)", end="")
        else:
